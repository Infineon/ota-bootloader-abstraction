...
sed -i '/#define/d' "$output_mk_file"

# Replace 'MCUBOOT_APP' with 'FLASH_AREA_IMG' in the output file
sed -i 's/MCUBOOT_APP\([0-9]*_\)/FLASH_AREA_IMG_\1/g' "$output_mk_file"

# Replace 'MCUBOOT_' with 'CY_MCUBOOT_' in the output file
sed -i 's/MCUBOOT_/CY_IFX_MCUBOOT_/g' "$output_mk_file"

# Replace ' := ' with '=' in the output file
sed -i 's/ := /=/g' "$output_mk_file"
...
sed -i '/#include "memorymap.h"/d; /#include "flash_map_backend.h"/d' "$output_header_file"

# Rename COMPONENT_MW_IFX_MCUBOOT_PSE84 to COMPONENT_IFX_MCUBOOT in cy_flash_map.h
sed -i 's/COMPONENT_MW_IFX_MCUBOOT_PSE84/COMPONENT_IFX_MCUBOOT/g' "$output_header_file"

# Rename BOOT_IMAGE_NUMBER to CY_IFX_MCUBOOT_IMAGE_NUMBER in cy_flash_map.h
sed -i 's/BOOT_IMAGE_NUMBER/CY_IFX_MCUBOOT_IMAGE_NUMBER/g' "$output_header_file"
