...
if [ "$OTA_BUILD_POST_VERBOSE" == "1" ]
then
   echo "$CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM -p $OTA_APP_POLICY_PATH sign-image -H $MCUBOOT_HEADER_SIZE --align 1 -v $APP_BUILD_VERSION -S $FLASH_AREA_IMG_1_PRIMARY_SIZE -R 0xff --image $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.final.bin --output $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.signed.bin --key-path $MCUBOOT_KEY_DIR/$MCUBOOT_KEY_FILE --update-key-path $CY_OEM_PRIVATE_KEY --image-id $CY_SIGN_IMG_ID"
fi

if [ "$CY_IMG_PAD" == "1" ]
then
   $CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM -p $OTA_APP_POLICY_PATH \
                         sign-image -H $MCUBOOT_HEADER_SIZE --align 1 \
                         -v $APP_BUILD_VERSION -S $FLASH_AREA_IMG_1_PRIMARY_SIZE \
                         -R 0xff \
...
then
    ...
    if [ "$OTA_BUILD_POST_VERBOSE" == "1" ]
    then
       echo "$CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM encrypt --input $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.signed.bin --output $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.encrypted.bin --iv $FLASH_AREA_IMG_1_PRIMARY_START_CBUS --enckey $CY_ENC_KEY --nonce $CY_NONCE"
    fi
    $CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM encrypt --input $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.signed.bin \
                                      ...
                                      --iv $FLASH_AREA_IMG_1_PRIMARY_START_CBUS \
                                      --enckey $CY_ENC_KEY \
                                      --nonce $CY_NONCE
fi
...
if [ "$OTA_BUILD_POST_VERBOSE" == "1" ]
then
   echo "$CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM -p $OTA_APP_POLICY_PATH sign-image --image-format $CY_SIGN_TYPE -i $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.final.bin -o $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.upgrade_signed.bin --key-path $MCUBOOT_KEY_DIR/$MCUBOOT_KEY_FILE --update-key-path $CY_OEM_PRIVATE_KEY --slot-size $FLASH_AREA_IMG_1_PRIMARY_SIZE --align 1 --image-id $CY_SIGN_IMG_ID -v $APP_BUILD_VERSION --pad"
fi

if [ "$CY_IMG_PAD" == "1" ]
then
   $CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM -p $OTA_APP_POLICY_PATH \
                   sign-image --image-format $CY_SIGN_TYPE \
                   -i $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.final.bin \
                   -o $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.upgrade_signed.bin \
                   --key-path $MCUBOOT_KEY_DIR/$MCUBOOT_KEY_FILE \
                   --update-key-path $CY_OEM_PRIVATE_KEY \
                   --slot-size $FLASH_AREA_IMG_1_PRIMARY_SIZE \
                   --align 1 \
                   --image-id $CY_SIGN_IMG_ID \
                   -v $APP_BUILD_VERSION --pad
else
   $CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM -p $OTA_APP_POLICY_PATH \
                   sign-image --image-format $CY_SIGN_TYPE \
                   -i $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.final.bin \
...
then
    ...
    if [ "$OTA_BUILD_POST_VERBOSE" == "1" ]
    then
        echo "$CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM encrypt --input $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.upgrade_signed.bin --output $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.upgrade_encrypted.bin --iv $FLASH_AREA_IMG_1_SECONDARY_START_CBUS --enckey $CY_ENC_KEY --nonce $CY_NONCE"
    fi
    $CY_PYTHON_PATH -m cysecuretools -q -t $APP_PLATFORM encrypt --input $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.upgrade_signed.bin \
                                      --output $CY_OUTPUT_PATH/$CY_OUTPUT_NAME.upgrade_encrypted.bin \
                                      --iv $FLASH_AREA_IMG_1_SECONDARY_START_CBUS \
                                      --enckey $CY_ENC_KEY \
                                      --nonce $CY_NONCE
fi
